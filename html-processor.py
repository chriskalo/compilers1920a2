...
rexp7 = re.compile(r'\s+') #teriasma akolouthiwn sinexomenwn xarakthrwn whitespace mesa sto keimeno
...
with open('testpage.txt','r') as fp:
  ...
  for m in rexp4.finditer(text): #anazhthsh kathe pithanou teriasmatos mesa sto keimeno simfona me thn mhxanh teriasmatos rexp4(parapanw perigrafetai h kanonikh ekfrash vash ths opoias kanei anazhthsh tairiasmatos h rexp4)
    print('{}    {}'.format(m.group(1),m.group(2))) #ektiposh merous tou kathe teriasmatos(sindesmos ths idiothtas href kai periexomeno metaksi <a> kai </a>) pou vrike h mhxanh teriasmatos rexp4

  text = rexp5_1.sub(' ',text) #apaloifh olwn twn tag tou keimenou me morfh p.x <a> h </a>
  text = rexp5_2.sub(' ',text) #apaloifh olwn twn tag tou keimenou me morfh p.x <meta charset="utf-8" />
  ...
  text = rexp7.sub(' ',text) #antikatastash kathe akolouthias sinexomenwn xarakthrwn whitespace tou keimenou me ena xarakthra whitespace
